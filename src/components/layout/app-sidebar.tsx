...
export default function AppSidebar() {
  ...
  return (
    /* Outer aside always holds w-14 in the layout — content never shifts */
    <aside
      className="relative w-14 shrink-0 h-full"
      onMouseEnter={() => setIsExpanded(true)}
      onMouseLeave={() => setIsExpanded(false)}
    >
      {/* Inner panel expands as overlay */}
      <div
        className={`absolute top-0 left-0 h-full flex flex-col overflow-hidden transition-all duration-200 z-40 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 ${
